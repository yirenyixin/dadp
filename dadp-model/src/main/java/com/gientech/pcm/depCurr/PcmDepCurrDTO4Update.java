...
@Data
@ApiModel(value = "对私活期存款--更新DTO")
public class PcmDepCurrDTO4Update implements Serializable {
    ...
    @ApiModelProperty(value = "ACCT_NO", required = true, position = 8)
    @NotBlank(message = "[acctNo] 账号不能为空")
    @Size(max = 32, message = "账号的长度必须小于等于32")
    private String acctNo; // 账号

    @ApiModelProperty(value = "SUB_ACCT_NO", required = true, position = 9)
    @NotBlank(message = "[subAcctNo] 子账号不能为空")
    @Size(max = 32, message = "子账号的长度必须小于等于32")
    private String subAcctNo; // 子账号

    @ApiModelProperty(value = "ACCT_TYPE", required = true, position = 10)
    ...
    @ApiModelProperty(value = "END_DATE", required = true, position = 16)
    ...
    @ApiModelProperty(value = "CURRENT_BAL", position = 22)
