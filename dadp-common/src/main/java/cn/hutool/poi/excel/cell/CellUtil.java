 ...
 * Excel表格中单元格工具类
 * 
 * 胡砥峰【在hutool5.7.5版本，修改了236行，解决了导出的日期型和时间型格式。】
 *
 * @author looly
 * @since 4.0.7
 */
public class CellUtil {
	...
	public static void setCellValue(Cell cell, Object value) {
		...
		if (null == value) {
			...
			// 胡砥峰修改20210706
			String dateStr = DateUtil.formatDateTime((Date) value);// 格式 yyyy-MM-dd HH:mm:ss
			if (StrUtil.endWith(dateStr, "00:00:00")) {
...
//			cell.setCellValue((Date) value);
	...
	public static Cell getMergedRegionCell(Cell cell) {
		...
		return ObjectUtil.defaultIfNull(getCellIfMergedRegion(cell.getSheet(), cell.getColumnIndex(), cell.getRowIndex()), cell);
	}
	...
	public static Cell getMergedRegionCell(Sheet sheet, int x, int y) {
		return ObjectUtil.defaultIfNull(getCellIfMergedRegion(sheet, x, y), SheetUtil.getCell(sheet, y, x));
	}
